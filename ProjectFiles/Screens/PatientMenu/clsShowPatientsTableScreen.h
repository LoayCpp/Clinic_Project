...
public:
	static void ShowPatientsTable()
	{
		...
		string title = clsUtil::Tabs(1) + "		 Show Patient Table Screen";
		string subTitle = clsUtil::Tabs(2) + "List of All Patient in the System (" + to_string(mPatient.size()) + " Patients)";

		ShowMainHeader(title, subTitle, 2);
		DrawTable(mPatient);
	}
};